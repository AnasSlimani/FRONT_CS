  ...
  useEffect(() => {
    if (activity && isOpen) {
      ...
      setFormData({
        ...
        startingDate: formatDate(activity.startingDate),
        endingDate: formatDate(activity.endingDate),
        time: activity.time || "",
        localisation: activity.localisation || "",
        nbrParticipants: activity.nbrParticipants?.toString() || "",
        nbrPerTeam: activity.nbrPerTeam?.toString() || "",
        nbrTeams: activity.nbrTeams?.toString() || "",
      ...
      if (activity.image) {
        setImagePreview(`/images/${activity.image}`)
      } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const activityData = {
        ...
        nbrCurrentParticipants: activity.nbrCurrentParticipants || 0,
        isTournamentFull: activity.isTournamentFull || false,
        nbrCurrentTeam: activity.nbrCurrentTeam || 0,
        teamParticipants: activity.teamParticipants || [],
        individualParticipants: activity.individualParticipants || [],
      }
      ...
      setError(error.message || "Failed to update activity. Please try again.")
    } finally {
      setIsLoading(false)
    }
  }
